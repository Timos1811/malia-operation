...
import { Loader2, CheckCircle2, ShieldAlert, XCircle, PartyPopper } from "lucide-react";
...
export default function NewSale() {
  ...
  // מנעול קריטי למניעת רישום כפול של אותו צמיד ברצף
  ...
  const handleNFCScan = async () => {
    if (isAllWristbandsScanned || !('NDEFReader' in window) || !formData.orderNumber) {
      return toast.error("בדוק מספר הזמנה או תמיכת NFC");
    }
    ...
    try {
      ...
      ndef.onreading = async (event) => {
        ...
        // הגנה 1: מניעת כפילות בתוך הזיכרון של הדף הנוכחי
        if (scannedIds.has(nfcId)) {
          toast.warning("צמיד זה כבר נסרק עבור הזמנה זו");
          return;
        }

        // הגנה 2: מנעול אטומי - מונע משתי קריאות NFC לרוץ במקביל לשרת
        if (isProcessingRef.current) return;
        isProcessingRef.current = true;

        try {
          // הגנה 3: בדיקה מול בסיס הנתונים - האם הצמיד משויך להזמנה כלשהי?
          // זו הבדיקה שמונעת את המצב שראית בתמונה
          const existingWristbands = await base44.entities.Wristband.list({
            filter: { nfc_id: nfcId }
          });

          if (existingWristbands.length > 0) {
            toast.error(`הצמיד כבר בשימוש! (הזמנה ${existingWristbands[0].order_number})`, {
              duration: 5000,
          ...
          // רישום הצמיד רק אם הוא נמצא פנוי לחלוטין
  ...
  if (isSuccess) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-6 text-center" dir="rtl">
        ...
        <h2 className="text-3xl font-black">מכירה בוצעה!</h2>
        <p className="text-slate-500 mb-8">כל הצמידים שויכו להזמנה {formData.orderNumber}</p>
        <Button className="w-full max-w-xs py-8 bg-slate-900 rounded-3xl" onClick={() => window.location.reload()}>מכירה חדשה</Button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-80" dir="rtl">
      ...
      <div className="p-4 max-w-md mx-auto space-y-6">
        <Card className="border-none shadow-md rounded-3xl p-5 bg-white space-y-4">
